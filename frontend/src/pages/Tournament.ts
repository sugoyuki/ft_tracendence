...
export default function Tournament(): HTMLElement {
  ...
  loadingIndicator.innerHTML = '<div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-gray-300 border-t-primary"></div><p class="ml-3">Loading tournaments...</p>';
  ...
  function createTournamentCard(tournament: any) {
    ...
    // Progress bar for registration (if in registration phase)
    ...
    // Add view action to button (except for registration actions)
  ...
  function openNewTournamentModal() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        // Close modal and refresh tournaments list
        modal.remove();
        tournamentGrid.innerHTML = '';
        tournamentGrid.appendChild(loadingIndicator);
        fetchActiveTournaments();
