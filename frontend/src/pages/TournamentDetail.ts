...
export default function TournamentDetail(): HTMLElement {
  ...
  loadingIndicator.innerHTML =
    '<div class="inline-block animate-spin rounded-full h-8 w-8 border-4 border-gray-300 border-t-primary"></div><p class="ml-3">Loading tournament details...</p>';
  ...
  function updateDetailsList(tournament: any) {
    ...
    const details = [
      {
        label: "Status",
        value: tournament.status.charAt(0).toUpperCase() + tournament.status.slice(1).replace("_", " "),
      },
  ...
  function updateActionButtons(tournament: any) {
    ...
    if (
      tournament.createdBy === user?.username &&
  ...
  function createMatchCard(match: any, isFinal: boolean) {
    ...
    player1.className = `flex justify-between items-center p-2 rounded ${
      match.winner === match.player1?.id ? "bg-green-900" : ""
    }`;
    ...
    player2.className = `flex justify-between items-center p-2 rounded ${
      match.winner === match.player2?.id ? "bg-green-900" : ""
    }`;
